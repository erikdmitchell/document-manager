...
class DM_Document_Download {
    ...
    public function handle_download_via_post() {
        ...
        if ( ! isset( $_REQUEST['dm_document_download'] ) || ! wp_verify_nonce( sanitize_key( $_REQUEST['dm_document_download'] ), 'process_download' ) ) { // Input var okay.
            return;
        }

        $dm_document_download_logging = true;
        $dispatch                     = true;
        $document_id                  = absint( $_REQUEST['document_id'] ); // Input var okay.
        $document_title               = dm_get_document_title( $document_id );
        $document_url                 = wp_get_attachment_url( $document_id );
        $redirect_url                 = home_url( wp_unslash( $_REQUEST['http_referer'] ) ); // Input var okay.
    ...
    protected function get_ip_address( $ignore_private_and_reserved = false ) {
        ...
        foreach ( array( 'HTTP_CLIENT_IP', 'HTTP_X_FORWARDED_FOR', 'HTTP_X_FORWARDED', 'HTTP_X_CLUSTER_CLIENT_IP', 'HTTP_FORWARDED_FOR', 'HTTP_FORWARDED', 'REMOTE_ADDR' ) as $key ) :
            if ( array_key_exists( $key, $_SERVER ) === true ) : // Input var okay.
                foreach ( explode( ',', $_SERVER[ $key ] ) as $ip ) : // Input var okay.
     ...
     *
     * @access protected
     * @param mixed $filename file path.
     * @return can return false or will force file download
     */
    ...
    protected function redirect_to_url( $url, $delay = '0', $exit = '1' ) {
        if ( empty( $url ) ) :
            echo '<strong>';
                esc_html_e( 'Error! The URL value is empty. Please specify a correct URL value to redirect to!', 'document-manager' );
            echo '</strong>';
        ...
        if ( ! headers_sent() ) :
            ...
            echo '<meta http-equiv="refresh" content="' . $delay . ';url=' . esc_url( $url ) . '" />';
        endif;
